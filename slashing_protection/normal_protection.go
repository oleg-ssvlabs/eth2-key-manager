...
func (protector *NormalProtection) IsSlashableAttestation(pubKey []byte, attestation *phase0.AttestationData) (*core.AttestationSlashStatus, error) {
	...
	if highest != nil {
		...
		fmt.Printf("DEBUGGING_STUFF: ATTESTATION SOURCE EPOCH: %d, VALIDATOR SOURCE EPOCH: %d, ATTESTATION TARGET EPOCH: %d, VALIDATOR TARGET EPOCH: %d",
			attestation.Source.Epoch, highest.Source.Epoch, attestation.Target.Epoch, highest.Target.Epoch)

		//NOTE: this check returns true because highest.Source.Epoch is uint64 max
